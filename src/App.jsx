...
const chip = (on, col) => ({
  background: on
    ? (col||T.purpleD)+"66"
    : T.isDark ? "#1e1e3a" : "#ede8dc",
  border: `1.5px solid ${on ? (col||T.purpleL) : T.isDark ? "#3a3a6a" : "#c0b8a0"}`,
  borderRadius: 22,
  padding: "8px 16px",
  color: on
    ? (col||T.purpleL)
    : T.isDark ? "#e8e4ff" : "#1a1410",
  fontSize: 13,
  cursor: "pointer",
          ...
          {TECNICAS.map(t=>(
            <div key={t.id} style={{background:T.isDark?"#13132a":"#ffffff",border:`1px solid ${t.color}55`,borderLeft:`4px solid ${t.color}`,borderRadius:14,padding:"16px",marginBottom:10,cursor:"pointer"}} onClick={()=>setTechDetail(t)}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:16,fontWeight:800,marginBottom:3,color:T.isDark?"#ffffff":"#0a0820"}}>{t.name}</div>
